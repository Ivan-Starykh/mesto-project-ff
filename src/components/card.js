...
// export function createCard(card, handleCardDelete, handleCardLike, openImagePopupCallback) {
...
export function createCard(card, deleteCardCallback, openImagePopupCallback, handleCardLikeCallback) {
  const cardElement = cardTemplate
    .querySelector(".places__item")
    .cloneNode(true);
  cardElement.querySelector(".card__title").textContent = card.name;
  ...
  const likeButton = cardElement.querySelector(".card__like-button");
  likeButton.addEventListener("click", () => handleCardLikeCallback(likeButton));
